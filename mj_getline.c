...
check_pipeline(info, buf, &m, i, len);
while (m < len) /* Iterate to the next semicolon or end of buffer. */
{
if (detect_chain(info, buf, &m))
break;
m++;
}
i = m + 1; /* increment past nulled ';'' */
...
if (*i)
return (0);
num_read = read(info->readfd, buf, OUT_BUFF_SIZE);
if (num_read >= 0)
*i = num_read;
return (num_read);
}
