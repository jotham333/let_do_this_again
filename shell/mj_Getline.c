...
{
	// Initialize variables to hold number of bytes read and length of the input.
	ssize_t num_read = 0;
	size_t len_p = 0;
	...
	if (!*len)
	{
		...
		*buf = NULL;
		...
		// Read input from stdin using either the getline() function or a custom implementation.
		#if USE_GETLINE
 ...
 */
{
	// Initialize variables to hold the command buffer and its position in the input buffer.
	static char *buf;
	...
	// Read input from stdin into the buffer, buffering chained commands if necessary.
	...
	// If there are commands left in the buffer from a chained command, process them.
	if (len > 0)
	{
